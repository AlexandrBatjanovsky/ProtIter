...
amino_str = ('-', 'A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'Y', 'X')
...
def calc_msa(arr: np.ndarray, use_tensordot=True, is_reshaped=True) -> np.ndarray:
...
def main_run():
    ...
    #
    t1 = time.time()
    msa = calc_msa(align_raw['arr'], use_tensordot=True, is_reshaped=False)
    dt = time.time() - t1
